...
{id:"jee-2019-p1",title:"JEE Main 2019",subtitle:"Full Paper",year:2019,exam:"JEE",qs:90,marks:360,duration:180,subject:"Physics, Chemistry, Maths",students:5800},
...
export function calcAnalysis(answers:number[], qs:any[], paper:any){
 ...
 answers.forEach((a,i)=>{
  ...
  if(subWise[q.subject]) subWise[q.subject].t++; diffWise[q.difficulty].t++;
 })
 ...
 const shouldCoaching = score < 180 ? {need:true, type:"Professional coaching will help you", reason:"Structured guidance can add 40-60 marks", coaching:"Consider Allen, Resonance or PhysicsWallah - 12-15 hrs/week", improvement:"Potential +40-60 marks with coaching"} : {need:false, type:"You are on track", reason:"Self study with weekly doubt clearing is enough for now", coaching:"Continue self study + 1 doubt session per week", improvement:"Potential +12-19 marks with better strategy"}
 return {correct,wrong,unattempted,score,total,pct,potential,rank:`${rankLow.toLocaleString()} - ${rankHigh.toLocaleString()}`,rankLow,rankHigh,subWise,diffWise,shouldCoaching}
}
